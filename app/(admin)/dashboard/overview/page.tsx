...
import { CalendarIcon, TrendingUp, Package, ShoppingCart, DollarSign } from "lucide-react";
...
export default function DashboardChart() {
  ...
  const { data, isLoading, error } = useGetDashboardQuery({ start, end }, { skip: !start || !end });
  ...
  return (
    <div className="w-full mx-auto p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 space-y-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Báo cáo thống kê</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">Tổng quan doanh số và hoạt động kinh doanh</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gradient-to-r from-green-50 to-green-100 dark:from-green-900 dark:to-green-800 p-4 rounded-xl border border-green-200 dark:border-green-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-green-700 dark:text-green-300">Tổng sản phẩm</p>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 p-4 rounded-xl border border-blue-200 dark:border-blue-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-blue-700 dark:text-blue-300">Tổng đơn hàng</p>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900 dark:to-purple-800 p-4 rounded-xl border border-purple-200 dark:border-purple-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-purple-700 dark:text-purple-300">Tổng doanh thu</p>
      ...
      {error && (
        <div className="flex items-center justify-center h-64 bg-red-50 dark:bg-red-900 rounded-xl border border-red-200 dark:border-red-700">
          <div className="text-center">
            <p className="text-red-600 dark:text-red-400 font-medium">Có lỗi xảy ra khi tải dữ liệu</p>
            <p className="text-sm text-red-500 dark:text-red-300 mt-1">Vui lòng thử lại sau</p>
          </div>
        </div>
      )}
      ...
      {!isLoading && !error && chartData.length > 0 && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="mb-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Biểu đồ thống kê</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Dữ liệu từ {dateRange?.from && format(dateRange.from, "dd/MM/yyyy", { locale: vi })}
          ...
          <ChartContainer config={chartConfig} className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <XAxis dataKey="dateLabel" tick={{ fontSize: 12 }} className="text-gray-600 dark:text-gray-400" />
      ...
      {!isLoading && !error && chartData.length === 0 && (
        <div className="flex items-center justify-center h-64 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 font-medium">Không có dữ liệu</p>
            <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">Vui lòng chọn khoảng thời gian khác</p>
          </div>
        </div>
      )}
    </div>
